...
export default function MemberDashboard() {
  ...
  const activeUser = isDemo ? mockMemberUser : user;
  ...
  const [announcements] = useState(() => {
    ...
    if (saved) return JSON.parse(saved);
    return isDemo ? mockAnnouncements : [];
  });
        ...
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1.5 text-sm text-ink-soft">
              ...
              {activeUser?.department} · {activeUser?.year}
            </p>
          </div>
              ...
              <div className="space-y-3">
                {todayEvents.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center text-ink-soft bg-cream-100/20 rounded-xl border border-border-soft">
                    ...
                    <p className="text-sm font-medium">No upcoming events scheduled yet.</p>
                  </div>
                )}
              </div>
            </Card>
          ...
          <FadeIn delay={0.14}>
            <Card>
              <CardHeader
                ...
                action={<Badge tone="navy">{myRegisteredEvents.length} upcoming</Badge>}
              />
              <div className="space-y-3">
                {myRegisteredEvents.length > 0 ? (
                  myRegisteredEvents.map((e, i) => (
                    <motion.div
                      key={e.id}
                      initial={{ opacity: 0, x: -12 }}
                      animate={{ opacity: 1, x: 0 }}
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {blogPosts.length > 0 ? (
                  ...
                  <div className="col-span-2 flex flex-col items-center justify-center py-8 text-center text-ink-soft bg-cream-100/20 rounded-xl border border-border-soft">
                    <BookOpen className="h-8 w-8 text-ink-soft/40 mb-2" />
                    <p className="text-sm font-medium">No blog posts published yet.</p>
                  </div>
                )}
              </div>
            </Card>
          ...
          <FadeIn delay={0.1}>
            <Card>
              <div className="flex flex-col items-center p-2 text-center">
                <Avatar name={activeUser?.name ?? 'User'} src={activeUser?.avatarUrl} size="xl" ring className="shadow-card" />
                <h3 className="mt-3 text-base font-bold text-ink">{activeUser?.name}</h3>
                <p className="text-xs text-ink-soft">{activeUser?.department} · {activeUser?.year}</p>
                <div className="mt-2 flex flex-wrap justify-center gap-1.5">
                  {activeUser?.badges && activeUser.badges.length > 0 ? (
                    activeUser.badges.slice(0, 2).map((b) => (
                      <Badge key={b.id} tone="neutral">{b.label}</Badge>
                    ))
                  ) : (
                    <span className="text-xs text-ink-soft">No badges earned yet</span>
                  )}
                </div>
                <div className="mt-4 grid w-full grid-cols-3 gap-2 border-t border-border-soft pt-4">
                  {[
                    { label: 'Rank',   val: `${myRank === '-' ? '-' : '#' + myRank}`, icon: Trophy },
                    { label: 'Points', val: `${myPoints}`,              icon: Star   },
                    { label: 'Certs',  val: `${activeUser?.certificates?.length ?? 0}`, icon: Award  },
                  ].map((s) => (
                ...
                {leaderboard.length > 0 ? (
                  leaderboard.slice(0, 5).map((p, i) => (
                    <motion.div
                      ...
                      className={`flex items-center gap-3 rounded-lg p-2 ${p.rank === Number(myRank) ? 'bg-navy/[0.04] ring-1 ring-navy/15' : ''}`}
                    >
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center text-ink-soft bg-cream-100/20 rounded-xl border border-border-soft">
                    <Trophy className="h-8 w-8 text-ink-soft/40 mb-2" />
                    <p className="text-sm font-medium">No contributors on the leaderboard yet.</p>
                  </div>
                )}
              </div>
            </Card>
            ...
            <Card>
              ...
              <div className="space-y-3">
                {announcements.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center text-ink-soft bg-cream-100/20 rounded-xl border border-border-soft">
                    <Bell className="h-8 w-8 text-ink-soft/40 mb-2" />
            ...
            <Card>
              ...
              <div className="space-y-2.5">
                {activeUser?.achievements && activeUser.achievements.length > 0 ? (
                  activeUser.achievements.map((a, i) => (
                    <motion.div
                      key={a.id}
                      initial={{ opacity: 0, x: 10 }}
                      animate={{ opacity: 1, x: 0 }}
