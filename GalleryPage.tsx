...
export default function GalleryPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const defaultItems = user?.id === 'demo' ? defaultGalleryItems : [];
      setItems(defaultItems);
      localStorage.setItem('campusos_gallery_bento', JSON.stringify(defaultItems));
    }
  }, [user]);
      ...
      {filteredItems.length === 0 ? (
        <FadeIn delay={0.08}>
          <div className="card-surface p-12 text-center max-w-lg mx-auto mt-8 flex flex-col items-center">
            ...
            <h3 className="text-lg font-bold text-ink mb-2">No Gallery Photos</h3>
            <p className="text-sm text-ink-soft mb-6 leading-relaxed">
              No photos have been uploaded to the campus gallery yet. Upload some memorable moments to get started!
            </p>
          </div>
        </FadeIn>
      ) : (
