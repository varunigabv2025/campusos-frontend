...
export default function FacultyDashboard() {
  ...
  const activeUser = isDemo ? mockFacultyUser : user;
  ...
  const stats = isDemo ? mockFacultyStats : [
    { id: 'fs1', label: 'Active Clubs', value: 0, icon: 'Layers', accent: 'navy' as const },
    { id: 'fs2', label: 'Total Members', value: 0, icon: 'Users', accent: 'success' as const },
    { id: 'fs3', label: 'Pending Approvals', value: 0, icon: 'Clock', accent: 'warning' as const },
    { id: 'fs4', label: 'Events This Month', value: 0, icon: 'CalendarDays', accent: 'sand' as const },
  ];

  const clubOverviews = isDemo ? mockClubOverviews : [];
  const pendingApprovals = isDemo ? mockPendingApprovals : [];
  const reports = isDemo ? mockReports : [];
  const timeline = isDemo ? mockFacultyTimeline : [];
        ...
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1.5 text-sm text-ink-soft">
              {activeUser?.designation ?? 'Faculty Coordinator'} · {activeUser?.department}
              {activeUser?.overseesClubs && (
                <> · Overseeing <span className="font-medium text-ink">{activeUser.overseesClubs.length} clubs</span></>
              )}
            </p>
          </div>
          ...
          <FadeIn delay={0.1}>
            <Card>
              ...
              <div className="space-y-3">
                {clubOverviews.length > 0 ? (
                  clubOverviews.map((club, i) => (
                    <motion.div
                      key={club.id}
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center text-ink-soft bg-cream-100/20 rounded-xl border border-border-soft">
                    <Layers className="h-8 w-8 text-ink-soft/40 mb-2" />
                    <p className="text-sm font-medium">No clubs under coordination yet.</p>
                  </div>
                )}
              </div>
            </Card>
              ...
              <div className="space-y-3">
                {pendingApprovals.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center text-ink-soft bg-cream-100/20 rounded-xl border border-border-soft">
                    ...
                    <p className="text-sm font-medium">No pending club approvals.</p>
                  </div>
                )}
              </div>
            </Card>
            ...
            <Card>
              ...
              <div className="space-y-3">
                {reports.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center text-ink-soft bg-cream-100/20 rounded-xl border border-border-soft">
                    <FileText className="h-8 w-8 text-ink-soft/40 mb-2" />
