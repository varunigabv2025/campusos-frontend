...
export default function LeadDashboard() {
  ...
  const activeUser = isDemo ? mockLeadUser : user;
  ...
  const [events] = useState(() => {
    ...
    return isDemo ? mockEvents : [];
  });
  const [projects] = useState(() => {
    const saved = localStorage.getItem('campusos_projects');
    if (saved) return JSON.parse(saved);
    return isDemo ? mockProjects : [];
  });
  const calendar = buildMockCalendar(events);

  const stats = isDemo ? mockLeadStats : [
    { id: 'ls1', label: 'Team Members', value: 0, icon: 'Users', accent: 'navy' as const },
    { id: 'ls2', label: 'Active Projects', value: 0, icon: 'FolderKanban', accent: 'success' as const },
    { id: 'ls3', label: 'Events This Month', value: 0, icon: 'CalendarDays', accent: 'sand' as const },
    { id: 'ls4', label: 'Pending Approvals', value: 0, icon: 'Clock', accent: 'warning' as const },
  ];

  const pendingApprovals = isDemo ? mockPendingApprovals : [];
  const recentRegistrations = isDemo ? mockRecentRegistrations : [];
  const timeline = isDemo ? mockTimeline : [];
  const todayEvents = isDemo ? mockTodayEvents : [];
  const clubMembers = isDemo ? mockClubMembers : [];
  const teamSize = isDemo ? (activeUser?.teamSize ?? 24) : 0;
        ...
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1.5 text-sm text-ink-soft">
              {activeUser?.club && <><span className="font-medium text-ink">{activeUser.club}</span> · </>}
              {teamSize} members · {activeUser?.department}
            </p>
          </div>
          ...
          <FadeIn delay={0.1}>
            <Card>
              <CardHeader
                ...
                action={<Badge tone="warning" dot>{pendingApprovals.length} pending</Badge>}
              />
              <div className="space-y-3">
                {pendingApprovals.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center text-ink-soft bg-cream-100/20 rounded-xl border border-border-soft">
                    <AlertCircle className="h-8 w-8 text-ink-soft/40 mb-2" />
            ...
            <Card>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {projects.length > 0 ? (
                  ...
                  <div className="col-span-2 flex flex-col items-center justify-center py-8 text-center text-ink-soft bg-cream-100/20 rounded-xl border border-border-soft">
                    <FolderKanban className="h-8 w-8 text-ink-soft/40 mb-2" />
                    <p className="text-sm font-medium">No active club projects yet.</p>
                  </div>
                )}
              </div>
            </Card>
            ...
            <Card>
              ...
              <div className="space-y-2.5">
                {recentRegistrations.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center text-ink-soft bg-cream-100/20 rounded-xl border border-border-soft">
                    <Users className="h-8 w-8 text-ink-soft/40 mb-2" />
          ...
          <FadeIn delay={0.22}>
            <Card>
              ...
              <div className="relative space-y-5 pl-2">
                {timeline.length > 0 && <div className="absolute bottom-2 left-[14px] top-2 w-px bg-border-soft" />}
                {timeline.length > 0 ? (
                  timeline.map((t, i) => (
                    <motion.div
                      key={t.id}
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center text-ink-soft bg-cream-100/20 rounded-xl border border-border-soft">
                    <Activity className="h-8 w-8 text-ink-soft/40 mb-2" />
            ...
            <Card>
              ...
              {clubMembers.length > 0 && (
                <div className="mt-3 flex items-center gap-2 border-t border-border-soft pt-3">
                  ...
                  {teamSize > 4 && (
                    <span className="text-xs text-ink-soft">+{teamSize - 4} more members</span>
                  )}
                </div>
              )}
            </Card>
            ...
            <Card>
              ...
              <div className="space-y-2.5">
                {todayEvents.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center text-ink-soft bg-cream-100/20 rounded-xl border border-border-soft">
                    <CalendarDays className="h-8 w-8 text-ink-soft/40 mb-2" />
