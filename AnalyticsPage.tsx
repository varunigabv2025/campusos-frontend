...
import { mockClubOverviews, isDemoMode } from '../../utils/mockData';

export default function AnalyticsPage() {
  const isDemo = isDemoMode();
  const activeClubsCount = isDemo ? mockClubOverviews.filter((c) => c.status === 'active').length : 0;
  const totalMembersCount = useCountUp(0, isDemo ? 248 : 0, true);
  ...
  const totalPoints = useCountUp(0, isDemo ? 12850 : 0, true);
  ...
  const trendData = isDemo ? [
    ...
    { month: 'Jan', regs: 0, att: 0 },
    { month: 'Feb', regs: 0, att: 0 },
    { month: 'Mar', regs: 0, att: 0 },
    { month: 'Apr', regs: 0, att: 0 },
    { month: 'May', regs: 0, att: 0 },
    { month: 'Jun', regs: 0, att: 0 },
  ];
    ...
    <div className="space-y-6">
      ...
      <StaggerGroup className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          { label: 'Total Points Awarded', value: typeof totalPoints === 'number' ? `${totalPoints} pts` : `${totalPoints}`, icon: Star, color: 'bg-warning/15 text-[#b07314]' },
        ].map((stat) => (
        ...
        <FadeIn delay={0.16}>
          <Card>
            ...
            <div className="space-y-3.5 pt-2">
              {mockClubOverviews.length === 0 ? (
                <div className="text-center py-8 text-xs text-ink-soft">
                  No club rankings available.
                </div>
              ) : (
                mockClubOverviews
                  .slice()
